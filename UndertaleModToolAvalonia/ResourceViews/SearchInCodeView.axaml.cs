...
public partial class SearchInCodeView : UserControl
{
    public SearchInCodeView()
    {
        InitializeComponent();
    ...
    private void DataGridRow1_Open_Click(object? sender, RoutedEventArgs e)
    {
        if (DataContext is SearchInCodeViewModel vm)
            if (e.Source is Control control)
                if (control.DataContext is SearchInCodeViewModel.SearchResult searchResult)
    ...
    private void DataGridRow1_OpenInNewTab_Click(object? sender, RoutedEventArgs e)
    {
        if (DataContext is SearchInCodeViewModel vm)
            if (e.Source is Control control)
                if (control.DataContext is SearchInCodeViewModel.SearchResult searchResult)
